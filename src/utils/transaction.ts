...
export async function sendAndConfirmTransactionWithRetries(
  ...
  for (let i = 0; i < transactionRetryNumber; i++) {
    ...
    if (result?.status == 'Unknown') {
      spinner.fail(
        `Unknown Signature Status with Signature ${result.signature}. The operation status is unknown. Please check the transaction status manually on the block explorer.`,
      );
    }
  }
