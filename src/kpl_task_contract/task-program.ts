...
export async function createTask(
  ...
  export async function Whitelist(
    ...
    let tx = await allowListInstruction({is_allowlisted: 1}, {
        manager: programKeypair.publicKey,
        taskState: taskStateInfoAddress,
        programId
    });
    await sendAndConfirmTransaction(connection, new Transaction().add(tx), [programKeypair]);
  }
  ...
  export async function Blacklist(
    ...
    let programKeypair = await createKeypairFromFile(PROGRAM_KEYPAIR_PATH);
    let tx = await allowListInstruction({is_allowlisted: 0}, {
        manager: programKeypair.publicKey,
        taskState: taskStateInfoAddress,
        programId
    });
