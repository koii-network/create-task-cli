...
export async function createTaskInstruction(
  ...
  const total_bounty_amount_in_roe =
    params.total_bounty_amount * 10 ** params.mint_digits;
  const bounty_amount_per_round_in_roe =
    params.bounty_amount_per_round * 10 ** params.mint_digits;

  const instructionData = {
    instruction: TaskInstruction.CreateTaskInstruction,
    task_name: new TextEncoder().encode(
      padStringWithSpaces(params.task_name, 24),
    ),
    task_description: new TextEncoder().encode(
      padStringWithSpaces(params.task_description, 64),
    ),
    task_audit_program: new TextEncoder().encode(
      padStringWithSpaces(params.task_audit_program, 64),
    ), //must be 64 chracters long
    task_executable_network: new TextEncoder().encode(
      padStringWithSpaces(params.task_executable_network, 64),
    ), //must be 64 chracters long
    ...
    task_metadata: new TextEncoder().encode(
      padStringWithSpaces(params.task_metadata, 64),
    ),
    task_locals: new TextEncoder().encode(
      padStringWithSpaces(params.task_locals, 64),
    ),
