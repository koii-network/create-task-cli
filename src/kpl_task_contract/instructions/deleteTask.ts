...
export async function deleteTaskInstruction(
  ...
  const mint = await new PublicKey(accounts.mint_address || '');
  DataLayout.encode(
    {
  ...
  const [stakePotAuth, _bump] = await PublicKey.findProgramAddressSync(
    [seed],
    accounts.programId,
  );

  const [managerAccountDataHolder, _] = await PublicKey.findProgramAddressSync(
    [Buffer.from('manager')],
    accounts.programId,
  );
