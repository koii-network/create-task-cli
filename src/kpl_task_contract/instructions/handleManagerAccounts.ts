...
const DataLayout = struct<Data>([u8('instruction'), blob(24, 'operation_vec')]);
...
export async function handleManagerAccountsInstruction(
  params: HandleManagerAccountsParams,
  ...
  DataLayout.encode(
    {
      ...
      operation_vec: new TextEncoder().encode(
        padStringWithSpaces(params.operation, 24),
      ),
    },
    data,
  );
  const [managerAccountDataHolder, _bump] = await PublicKey.findProgramAddress(
    [Buffer.from('manager')],
    programId,
  );
  ...
  if (params.operation == 'init') {
    ...
    const SYSVAR_PUB = new PublicKey('11111111111111111111111111111111');
